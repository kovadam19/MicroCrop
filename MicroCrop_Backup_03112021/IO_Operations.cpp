...
void loadMaterialProperties(Settings& settings, MaterialContainer& materials)
{
	...
	for (auto& file_name : settings.MaterialFiles)
	{
		...
		while (MyFile >> item)
		{
			...
			// Check if the item is the ANISOTROPY_ULTIMATE_FORCES keyword
			if (item == "ANISOTROPY_ULTIMATE_FORCES")
			{
				// Read the anisotropy rotational stiffnesses
				for (int i = 0; i < number_of_materials; i++)
				{
					MyFile >> materials[i].ultimate_forces[0];
					MyFile >> materials[i].ultimate_forces[1];
					MyFile >> materials[i].ultimate_forces[2];
				}
			}
		}
