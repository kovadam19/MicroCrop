								 ...
								 MaterialContainer&			materials	)
{
	...
	for (int i = 0; i < particles.size(); i++)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			...
			int node_a_index = particles[i].intersections[2 * j];
			int node_b_index = particles[i].intersections[2 * j + 1];
			...
			particles[i].axial_springs[j] = axial_springs.size() - 1;
		}
	}
}
												...
												ParticleNode*		nodes)
{
	...
	for (int i = 0; i < number_of_springs; i++)
	{
		...
		if (axial_springs[i].status == 1)
		{
			...
			// Apply the forces on the particle nodes
