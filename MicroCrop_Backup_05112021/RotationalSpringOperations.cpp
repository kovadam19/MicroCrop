									  ...
									  MaterialContainer&			materials)
{
	...
	for (int i = 0; i < particles.size(); i++)
	{
		...
		for (int j = 0; j < 3; j++)
		{
			...
			int spring_a_index = particles[i].axial_springs[spring_a_location];
			int spring_b_index = particles[i].axial_springs[spring_b_location];
			...
			// Assign the particle to the spring
			new_spring.particle = i;
			...
			// Assign the spring to the particle
			particles[i].rotational_springs[j] = rotational_springs.size() - 1;
		}
	}
}
													 ...
													 ParticleNode*		nodes)
{
	...
	for (int i = 0; i < number_of_rsprings; i++)
	{
		...
		if (rotational_springs[i].status == 1)
		{
			...
			// Get the particle node coefficients
